    ...
    <table class="table table-bordered table-hover">
        ...
        <?php foreach($cuo->mostrarCuotas($CodAlumno) as $cuotas){
            ...
            <tr id="tr<?php echo $cuotas['CodCuota']?>" class="contenido <?php echo $cuotas['Cancelado']?'success':'';?>">
				...
				<td><input type="text" value="<?php if($cuotas['Cancelado'])echo date("d-m-Y",strtotime($cuotas['Fecha']));else echo date("d-m-Y");?>" size="6" id="fe<?php echo $cuotas['CodCuota'];?>"/ class="ku fechass input-small" rel="<?php echo $cuotas['CodCuota'];?>"></td>
            </tr>
            <?php	
		}?>
    </table>
