...
if(isset($_POST)){
	...
	$CodAumno=$_SESSION['CodUsuarioLog'];
    ...
    <?php if(count($cl)){?>
        ...
        ?>
            <tr class="<?php echo $c['CodAlumno']!=$CodAumno?'info':''?>">
                <td><?php echo $i;?></td>
                ...
                <hr class="separador">
                    <div class="mostrarHora centrar" title="<?php echo $idioma['FechaRegistro']?>"><?php echo fecha2Str($c['FechaRegistro'])." ".hora2Str($c['HoraRegistro'])?><?php if($c['CodAlumno']!="0"){?> - <?php echo capitalizar($al['Paterno']." ".$al['Materno']." ".$al['Nombres']);}else{?> - <?php echo $idioma['Docente'];}?></div>
                </td>
                <td>
                	<?php if($c['CodAlumno']==$CodAumno){?><a class="btn btn-mini eliminar" rel="<?php echo $c['CodClases']?>" title="<?php echo $idioma['Eliminar']?>">x</a><?php }?>
                </td>
            </tr>
        <?php }
